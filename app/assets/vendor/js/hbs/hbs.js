...
'Handlebars', 'lodash', 'hbs/i18nprecompile',
      ...
      return {
        ...
        load: function (name, parentRequire, load, config) {
          //>>excludeStart('excludeHbs', pragmas.excludeHbs)
            ...
            var compiledName = tmpl_root_path + name + customNameExtension,
              ...
              fetchText(path, function (text) {
                  ...
                  for ( var i in deps ) {
                    if ( deps.hasOwnProperty(i) ) {
                      try { deps[ i ] = 'hbs!' + deps[ i ].replace(/_/g, '/'); }
                      catch (e) {}
                    }
                  }
            ...
            if(omitExtension) {
              path = parentRequire.toUrl(tmpl_root_path + name);
            } else {
              path = parentRequire.toUrl(tmpl_root_path + name +'.'+ (config.hbs && config.hbs.templateExtension ? config.hbs.templateExtension : templateExtension));
            }
